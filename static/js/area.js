...
function changeAddress1(v1){
	var str="0";
	for(i=0;i<v1;i++){
		str+=("_"+(document.getElementById(s[i]).selectedIndex-1));
	};
	var ss=document.getElementById(s[v1]);
	with(ss){
		length = 0;
		options[0]=new Option(opt0[v1],opt0[v1]);
		if(v1 && document.getElementById(s[v1-1]).selectedIndex>0 || !v1){
			if(dsy.Exists(str)){
				ar = dsy.Items[str];
				for(i=0;i<ar.length;i++){
					options[length]=new Option(ar[i],ar[i]);
				}//end for
				if(v1){ options[0].selected = true; }
			}
		}//end if v
		if(++v1<s.length){changeAddress1(v1);}
	}//End with
}

function changeAddress2(v2){
	var str2="0";
	for(i=0;i<v2;i++){
		str2+=("_"+(document.getElementById(s2[i]).selectedIndex-1));
	};
	var ss2=document.getElementById(s2[v2]);
	with(ss2){
		length = 0;
		options[0]=new Option(opt0[v2],opt0[v2]);
		if(v2 && document.getElementById(s2[v2-1]).selectedIndex>0 || !v2){
			if(dsy.Exists(str2)){
				ar = dsy.Items[str2];
				for(i=0;i<ar.length;i++){
					options[length]=new Option(ar[i],ar[i]);
				}//end for
				if(v2){ options[0].selected = true; }
			}
		}//end if v
		if(++v2<s.length){changeAddress2(v2);}
	}//End with
}
...
var s=["old_province","old_city","old_county"];//三个select的name
var s2=["new_province","new_city","new_county"];//三个select的name
var opt0 = ["省","市","区、县"];//初始值
function _init_area(){  //初始化函数
	for(var i=0;i<2;i++){
	  document.getElementById(s[i]).onchange = new Function("changeAddress1("+(i+1)+")");
  	  document.getElementById(s2[i]).onchange = new Function("changeAddress2("+(i+1)+")");
	}
	changeAddress1(0);
	changeAddress2(0);
}
